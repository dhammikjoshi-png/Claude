...
const Sprites = {
  ...
  drawTree(ctx, x, y) {
    if (Assets.has("tree")) {
      const img = Assets.images.tree;
      const w = 46, h = 83; // drawn larger than the 16px tile grid on purpose —
      ctx.save();               // shrinking this art down to tile-size would blur out its detail
      ctx.fillStyle = "rgba(10,20,10,0.25)";
      ctx.beginPath();
      ctx.ellipse(x + w / 2, y + h - 4, w * 0.4, 5, 0, 0, Math.PI * 2);
      ctx.fill();
      ctx.drawImage(img, x - (w - 28) / 2, y - (h - 34), w, h);
      ctx.restore();
      return;
    }
    // Procedural fallback (used if the image hasn't loaded yet, or failed to)
    ctx.save();
    ctx.translate(Math.round(x), Math.round(y));
  ...
  drawFence(ctx, x, y) {
    if (Assets.has("fence")) {
      const w = 32, h = 15;
      ctx.drawImage(Assets.images.fence, x, y - 2, w, h);
      return;
    }
    ctx.save();
    ctx.translate(Math.round(x), Math.round(y));
